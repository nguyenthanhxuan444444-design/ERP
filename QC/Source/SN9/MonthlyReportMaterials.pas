...
type
  TMonthlyReportMaterial = class(TForm)
    ...
    Query2LotQty: TIntegerField;
    Query2DeLotQty: TIntegerField;
    Query2TotalQty: TIntegerField;
          ...
          usinserted:
            begin
              ...
              begin
                ...
                if Pos('/', Query2.FieldByName('DeQty').AsString) = 0 then
                  begin
                    ShowMessage('Khong tim thay ky tu "/" trong DeQty');
                    Exit;
                  end;
...
begin
  ...
  begin
    ...
    if ckMR.Checked then
      SQL.Add('and CAST(MonthReport as DATE) = ''' + FormatDateTime('yyyy-mm-dd', dtpMR.Date) + ''' ');
    Active := true;
  end;
end;
...
var
  ...
  i, p, N, SumVal, TotalRow, TotalColumn: Integer;
  MaxHeight: Double;
...
begin
  ...
  begin
    ...
    Worksheet.Cells[InsertRow, 3].Value := Query2.FieldByName('LotQty').AsString + 'PO';
    Worksheet.Cells[InsertRow, 4].Value := Query2.FieldByName('DeLotQty').AsString + 'PO';
    Worksheet.Cells[InsertRow, 5].Value := Query2.FieldByName('TotalQty').AsString + 'PRS';
    Worksheet.Cells[InsertRow, 6].Value := Query2.FieldByName('DeQty').AsString + 'PRS';
    ...
    begin
      ...
      begin
        Val(
          VarToStr(Worksheet.Cells[TotalRow, TotalColumn].Value),
          N,
          Err
        );
        SumVal := SumVal + N;
...
with Query2 do
  begin
    ...
    SQL.Add('SELECT ReportID, Supplier, CLBH, LotQty, DeLotQty, TotalQty, DeQty,');
    SQL.Add('CAST(');
    SQL.Add('  CAST(LEFT(DeQty, CHARINDEX(''/'', DeQty) - 1) AS FLOAT)');
    SQL.Add('  /');
    SQL.Add('  NULLIF(');
    SQL.Add('    CAST(SUBSTRING(DeQty, CHARINDEX(''/'', DeQty) + 1, LEN(DeQty)) AS FLOAT),');
    SQL.Add('    0');
    SQL.Add('  )');
    SQL.Add('  * 100');
    SQL.Add('  AS DECIMAL(10,1)');
    SQL.Add(') AS DeRate,');
    SQL.Add('Defects, Inspector, Remark, OKNG, USERDate, USERID, YN');
