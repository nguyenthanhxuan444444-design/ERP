...
type
  TDevQuota = class(TForm)
    ...
    DS8: TDataSource;
    UpdSQL: TUpdateSQL;
    Qry_CTseason: TStringField;
    Qry_CTCLBH: TStringField;
    Qry_CTGSBH: TStringField;
    Qry_CTZSBH: TStringField;
    Qry_CTSamplePrice: TFloatField;
    Qry_CTPriceFormula: TStringField;
    Qry_CTFreight: TStringField;
    Qry_CTarticle: TStringField;
    Qry_CTKFCQ: TStringField;
    Qry_CTKHDH: TStringField;
    Qry_CTXieXing: TStringField;
    Qry_CTSheHao: TStringField;
    Qry_CTYN: TStringField;
    btn1: TButton;
    Qry_CTUSERID: TStringField;
    Qry_CTUSERDATE: TDateTimeField;
    Delete2: TMenuItem;
    UpdateQuery1: TUpdateSQL;
...
begin
  ...
  begin
      ...
      begin
          KFCQ:='YIH';
          FTY:='LYN-9Z';   //«~µP©w¸q¦WºÙ  µL¸ê®Æ¥N¥ÎLYN
      end
      else if CB_FTY.ItemIndex=1 then
      begin
          KFCQ:='YQA';
          FTY:='LYI-6L';   //«~µP©w¸q¦WºÙ  µL¸ê®Æ¥N¥ÎLYN
      end
      else if CB_FTY.ItemIndex=2 then
      begin
          KFCQ:='TYB';
          FTY:='LYI-6L';   //«~µP©w¸q¦WºÙ  µL¸ê®Æ¥N¥ÎLYN
      end;
  ...
  begin
   ...
   begin
      ...
      begin
         ...
         begin
            if KFCQ='YQA' then
              sql.add('inner join XXZL with (nolock) on CostingPriceList.SKU+''-''+CostingPriceList.color_code+''(YQA)'' = XXZL.article ')
            else
         ...
         //24U
         SQL.Add('left join(select XXZLS.XieXing,XXZLS.SheHao,CostingPriceList.PricingSeason  ---,XXZLS.CLBH,MaterialCBD_His.PriceFormula ---ºû­×¬d§ä¤£¥¿±`¤Ï¬õ');
         ...
         begin
            if KFCQ='YQA' then
              SQL.Add('		  		  inner join XXZL with (nolock) on CostingPriceList.SKU+''-''+CostingPriceList.color_code+''(YQA)'' = xxzl.article  and XXZL.JiJie is not null ')
            else
         ...
         {SQL.Add('					union all ');  ²Ä¤T¼h¤l§÷®Æ¬d¸ß¦øªA¾¹ª¬ªp¤£¦nµLªk§¹¦¨- ¨Ï¥Î²v§CÃö³¬
        ...
        //20251216 checked with Kelly different KHDH same price
        // sql.add('and MaterialCBD_His.KHDH='''+KHDH+'''');
         ...
         if CheckBox5.Checked=true then   //¨Ï¥ÎªÌ¤W¶Ç
...
begin
 ...
 { if GSBH_SFL='PD' then
  ...
  begin
    Button9.Visible:=true;//¥þ³¡­«¶]¤À¬q§PÂ_
 ...
 if ((main.edit1.text='351205') or (main.edit1.text='316503')or (main.edit1.text='318738')or (main.edit1.text='341001')) then
...
if XXZL.Active=false then XXZL.Active:=true;
  ...
  begin
    ...
    sql.Add('and XXZLS.CLBH like'''+edit7.Text+'%''');  //§÷®Æ½s¸¹
    //20180515
    if ((checkbox3.Checked=true) and (checkbox4.Checked=false)) then
      sql.add('       and CLZL.CQDH=''TW''');                         //¼t°Ï¥N¸¹
    if ((checkbox3.Checked=false) and (checkbox4.Checked=true)) then
      sql.add('       and CLZL.CQDH=''VN''');
    if SupEdit.Text<>'' then
      sql.Add('      and XXZLS.CSBH '+GetSpeedButtonString(SBtn5,SupEdit.Text) );
    ...
    if BWLBCombo2.ItemIndex>0 then
      sql.Add('and (XXZLS.BWLB='+Copy(BWLBCombo2.Text,1,1)+' ) ');                     //³¡¦ìÃþ§O
...
begin
  ...
  begin
    ...
    sql.add('and XieXing not like ''COI%''');
    sql.add('left join KFXXZL on xxzl.XieXing=KFXXZL.XieXing and xxzl.SheHao=KFXXZL.SheHao ');
    ...
    sql.add('and XXZLS.CSBH<>''''');       //±Æ°£³¡¤À¦QµPCLZLµL«Ø¥ß¼t°Ó-«~µP´£¨Ñ
    if edit16.Text<>'' then
         ...
         with TemQry do
          begin
            ...
            begin
              ...
              TemQry1.Active:=True;
              if (TemQry1.RecordCount>0) and (main.Edit1.Text='000000')
                  and (TemQry.fieldbyname('SamplePrice').value<>Eval(Query.fieldbyname('PriceFormula').AsString))
                  and (Query.fieldbyname('CQDH').AsString='VN') then
              begin
                  if Isflag=true then
              ...
              begin
                ...
                begin
                  ...
                  //sql.Add(','''+main.edit2.Text+'''');     //«~µP¿ï¾Ü­×§ï¤£¦P¼t°Ï
                  if (CB_FTY.ItemIndex=1) and (main.Edit2.Text='CDC')then
                    sql.Add(',''VB1''')
                  else
...
begin
  if (main.Edit2.Text='CDC') or (main.Edit2.Text='VB1') then
      if Checkbox11.Checked then SaveBtn(XXMQ,Cancel2,Save2,false);
if main.Edit2.Text = 'VDH' then
begin
    with Query1 do
    begin
...
begin
 ...
 if  Query.active  then
    ...
    try
        WorkBook:=eclApp.workbooks.Add;
        eclApp.Cells(1,1):='»ù®æ®Ö­ã®Ñ';
        eclApp.Cells(2,1):='¼t°Ó';
        eclApp.Cells(2,2):='®Æ¸¹';
        eclApp.Cells(2,3):='­^¤å«~¦W';
//        eclApp.Cells(2,4):='¤¤¤å«~¦W';
        eclApp.Cells(2,4):='³æ¦ì';
        eclApp.Cells(2,5):='CBD³æ»ù';
        eclApp.Cells(2,6):='»ù®æ¤½¦¡';
        eclApp.Cells(2,7):='¹B¶O';
        eclApp.Cells(2,8):='Remark';
        eclApp.Cells(2,9):='¼t°Ó¥N½X';
       ...
       eclApp.Cells(j+2,1):='¹ô§O$=USD';
       eclApp.Cells(j+3,1):='¶×²v';
       eclApp.Cells(j+4,1):='½Æ®Ö';
       eclApp.Cells(j+4,3):='ªì®Ö';
       eclapp.columns.autofit;
...
begin
  ...
  begin
    ...
    sql.Add('   ,MaterialCBD_His.Season,max(xxzl.ARTICLE) as sku,MaterialCBD_His.Freight,MaterialCBD_His.XieXing,MaterialCBD_His.SheHao');
    sql.add('From MaterialCBD_His');
    SQL.Add('LEFT JOIN CLZL   ON MaterialCBD_His.CLBH = CLZL.cldh ');       //and MaterialCBD_His.ZSBH= CLZL.ZSDH   CLZL ¼t°Ó¸ê®Æ¤£§¹¾ã §R°£±ø¥ó
    ...
    if not CKKFCQ.Checked then SQL.Add('where CLZL.CLZMLB=''N''')//  and MaterialCBD_His.KFCQ='''+KFCQ+'''');
    else  SQL.Add('where CLZL.CLZMLB=''N'' and MaterialCBD_His.KFCQ='''+KFCQ+'''');
    ...
    sql.add('          ,ZSZL_DEV.DeliveryTerm,MaterialMOQ.Remark,MaterialCBD_His.Freight,MaterialCBD_His.XieXing,MaterialCBD_His.SheHao');
  ...
  begin
    ...
    begin
            ...
            begin
              ...
              begin
                  ...
                  if (tempStr[2] in ['0'..'9']) then
                    tempStr := Copy(tempStr, 1, 2)   // ²Ä¤G­Ó¦r¬O¼Æ¦r ¡÷ ¨ú«e¨â½X
                  else
                    tempStr := Copy(tempStr, 1, 1);  // ²Ä¤G­Ó¦r¤£¬O¼Æ¦r ¡÷ ¥u¨ú²Ä¤@½X
      ...
      begin
        ...
        if CheckBox10.Checked =true then
          showmessage('²Ä¤@¦¸§ä¨ì'+CLBH_QQ+';CCQQ:'+QQ+'ZSYWJC_QQ'+ZSYWJC_QQ);
        ...
        begin
          ...
          if (Query.FieldByName('CCQQ').IsNull) or (Query.FieldByName('CCQQ').AsString='') then//
            ...
            //§ä¨ì³ø»ù½X
               ...
               if (pos('LACE',Query.fieldbyname('YWSM').Value)>0) and (Query.fieldbyname('PriceFormula').asstring<>'') then
                  begin
                    ...
                    if (Length(lace_size) >= 2) and (lace_size[2] in ['0'..'9']) then
                      lace_size := Copy(lace_size, 1, 2)   // ²Ä¤G­Ó¦r¬O¼Æ¦r ¡÷ ¨ú«e¨â½X
                    else
                      lace_size := Copy(lace_size, 1, 1);  // ²Ä¤G­Ó¦r¤£¬O¼Æ¦r ¡÷ ¥u¨ú²Ä¤@½X
                 ...
                 if CheckBox10.Checked =true then
                    showmessage('¦Û°Ê¶ñ¾c±a:PriceFormula:'+PriceFormula);
               ...
               if CheckBox10.Checked =true then
                  showmessage('§ä¨ì³ø»ù°¨«ü¼Ð¤W²¾:CostingRemark:'+CostingRemark+'J:'+inttostr(j));
              Query.moveby(-j);
              break;
          end;
          Query.next;
      ...
      if NOT((Query.FieldByName('CCQQ').IsNull) or (Query.FieldByName('CCQQ').AsString='')) then
        if (ZSYWJC_QQ=Query.fieldbyname('ZSYWJC').AsString) then    //·s¼W±ø¥ó ¬Û¦P¼t°Ó
        if (CLBH_QQ=Query.fieldbyname('CLBH').AsString) then
        begin
            QQ:=QQ+ Query.fieldbyname('CCQQ').Value;
            if CheckBox10.Checked =true then
                  showmessage(Query.fieldbyname('CLBH').AsString+'CLBH='+CLBH_QQ+':§ä¨ì³ø»ù½X§÷®Æ§÷®Æ:QQ:'+QQ)
        end
        else if
            ...
            begin
              Query.Edit;
              if CheckBox10.Checked =true then
                    showmessage('§ä¨ì¤À¬q§÷®Æ:CostingRemark:'+CostingRemark);
              //¦Û°Ê¶ñ¤À¬q¾c±a
              if pos('LACE',Query.fieldbyname('YWSM').Value)>0  then
                begin
...
procedure TDevQuota.Button13Click(Sender: TObject);
begin
  GetSpeedButton();
  with Qry_CT do
  begin
    Active:=false;
    SQL.Clear;
    sql.add('select CostingPriceList.PricingSeason as  season--©u¸`');
    sql.add(',YWBZPO.CLBH--¥~½c½s¸¹');
    sql.add(',MaterialCBD_His.GSBH');
    sql.add(',MaterialCBD_His.ZSBH');
    sql.add(',MaterialCBD_His.SamplePrice');
    sql.add(',MaterialCBD_His.PriceFormula');
    sql.add(',MaterialCBD_His.Freight');
    sql.add(',Max(CostingPriceList.SKU+''-''+CostingPriceList.Color_Code) as article');
    sql.add(',CostingPriceList.KFCQ--¤u¼t');
    sql.add(',Max(CostingPriceList.KHDH) as KHDH--«È¤á½s¸¹');
    sql.add(',Max(ddzl.XieXing) as XieXing');
    sql.add(',Max(ddzl.SheHao) as SheHao--¾c«¬ ¦â¸¹');
    sql.add(',Max (MaterialCBD_His.YN) as YN');
    sql.add(',MaterialCBD_His.USERID');
    sql.add(',MaterialCBD_His.USERDATE');
    sql.add('From CostingPriceList');
    sql.add('left join DDZL on DDZL.article=CostingPriceList.SKU+''-''+CostingPriceList.Color_Code');
    sql.add('left join YWBZPO on YWBZPO.DDBH=DDZL.DDBH');
    sql.add('left join MaterialCBD_His on CostingPriceList.PricingSeason=MaterialCBD_His.season and MaterialCBD_His.CLBH=YWBZPO.CLBH and MaterialCBD_His.KFCQ=CostingPriceList.KFCQ');
    sql.add('where PricingSeason like '''+Edit40.text+'%''');
    //sql.add('and CostingPriceList.KFCQ='''+KFCQ+'''');
    if Edit39.Text<>'' then
      sql.add('and DDZL.BUYNO like '''+Edit39.Text+'%''');
    if Edit38.Text<>'' then
      sql.add('and YWBZPO.CLBH like '''+Edit38.Text+'%''');
    if Edit35.Text<>'' then
      sql.add('and DDZL.article like '''+Edit35.Text+'%''');
    sql.add('and YWBZPO.CLBH is not null');
    sql.add('group by YWBZPO.CLBH,CostingPriceList.KFCQ,CostingPriceList.PricingSeason');
    sql.add(',MaterialCBD_His.GSBH,MaterialCBD_His.ZSBH,MaterialCBD_His.SamplePrice,MaterialCBD_His.PriceFormula,MaterialCBD_His.Freight');              
    sql.add(',MaterialCBD_His.USERID');
    sql.add(',MaterialCBD_His.USERDATE');
    sql.add('order by YWBZPO.CLBH');
    //showmessage(SQL.text);
    Active:=True;
    cachedupdates:=true;
    requestlive:=true;
...
procedure TDevQuota.btn1Click(Sender: TObject);  
function Eval(expStr: string): string;
var js: OleVariant;
begin js := CreateOleObject('ScriptControl');
js.Language := 'JavaScript';
...
begin                         
  ...
  begin
    ...
    while not Eof do
    begin
        if fieldbyname('PriceFormula').AsString<>'' then
        begin
          Edit;
          fieldbyname('GSBH').value:=main.Edit2.Text;
          fieldbyname('USERID').value:=main.Edit1.Text;
          fieldbyname('ZSBH').value:='VR14';
          fieldbyname('YN').value:=0;
                                                       
          if (fieldbyname('PriceFormula').Value<>fieldbyname('PriceFormula').OldValue)
              or (fieldbyname('Freight').Value<>fieldbyname('Freight').OldValue) then
          if fieldbyname('SamplePrice').OldValue = null then
          begin
            //ShowMessage('ukinsert');
            fieldbyname('SamplePrice').value:=StrToFloat(Eval(fieldbyname('PriceFormula').AsString));
...
procedure TDevQuota.Delete2Click(Sender: TObject);
begin
  ...
  begin
    ...
    begin
      ...
      requestlive:=true;
      Edit;
      FieldByName('yn').value := '1';  // dánh d?u xoá
      Query1.Post;
    end;
    Cancel2.Enabled:=true;
    Save2.Enabled:=true;
