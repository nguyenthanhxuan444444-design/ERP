...
begin
  ...
  begin
    Active:=false;
    SQL.Clear;
    sql.add('select CostingPriceList.PricingSeason as  season--季節');
    sql.add(',YWBZPO.CLBH--外箱編號');
    ...
    sql.add(',CostingPriceList.KFCQ--工廠');
    sql.add(',Max(CostingPriceList.KHDH) as KHDH--客戶編號');
    sql.add(',Max(ddzl.XieXing) as XieXing');
    sql.add(',Max(ddzl.SheHao) as SheHao--鞋型 色號');
