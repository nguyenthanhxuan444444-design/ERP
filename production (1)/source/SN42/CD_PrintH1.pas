...
procedure TCD_PrintH.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  action:=cafree;
end;

procedure TCD_PrintH.FormDestroy(Sender: TObject);
begin
  CD_PrintH:=nil;
end;

procedure TCD_PrintH.Init(DLNO:string; IsRoundNum: boolean; IsShowCutNum: boolean; IsShowTotalPairs: boolean; depid: string; CustomLayers: integer);
var i,j:integer;
...
begin
  ...
  begin
    Active:=false;
    SQL.Clear;
    SQL.Add('select CutDispatchS.*,ZLZLS2.CLSL from ( ');
    if (CustomLayers = 0) then
    ...
    SQL.Add('Left join ( ');
    SQL.Add('Select CLBH,'+SubSQLStr4+' as LB,SUM(CLSL) as CLSL from zlzls2 ');
    SQL.Add('where exists (select ZLBH from CutDispatchS where DLNO='''+DLNO+''' and zlzls2.ZLBH=CutDispatchS.ZLBH and CutDispatchS.BWBH=CutDispatchS.BWBH and CutDispatchS.CLBH=ZLZLS2.CLBH) ');
    SQL.Add('Group by CLBH,'+SubSQLStr4+' ) ZLZLS2 on ZLZLS2.CLBH=CutDispatchS.cldh and ZLZLS2.LB=CutDispatchS.LB   ');
  ...
  begin
    ...
    begin
      ...
      if (IsShowTotalPairs) then
        SQL.Add(') AS CutDispatchS ORDER BY lb, YWSM');
      Active:=true;
    end;
  end else
  begin
    ...
    begin
      Active:=false;
      SQL.Clear;
      SQL.Add('select DDBH as ZLBH, YWSM, 1 AS LB');
      Qtemp.First;
      while not Qtemp.Eof do
      begin
        SQL.Add(',Max(Case when SIZE='''+Qtemp.FieldByName('SIZE').AsString+''' then Qty else null end) as '''+Qtemp.FieldByName('SIZE').AsString+''' ');
