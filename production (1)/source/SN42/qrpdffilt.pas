...
begin
  ...
  if ir.ItemType=IT_TEXT then begin
    ...
    if FontHandling in [fhAutoEmbed, fhNoEmbedding] then AddTTFont(PDFFontName);
...
var
  ...
  type
    ...
    var
      ...
      begin
        ...
        for K := 0 to PDFFont.NumMetrics-1 do begin
          PDFFont.Metrics[K]:=(256*Buff[K*4])+(Buff[K*4+1]);//cvtInt(Buff, Offset+(K*4));
    ...
    begin
      ...
      try
        ...
        if res=GDI_ERROR then raise EQRError.Create('GetFontData error');
        ntabs:=(256*Buff[4])+(Buff[4+1]);//cvtInt(Buff, 4);
        for K := 0 to ntabs-1 do begin
          ...
          tabTag:=Chr(Buff[Offset])+Chr(Buff[Offset+1])+Chr(Buff[Offset+2])+Chr(Buff[Offset+
                                 3]);
          toff:=(256*256*256*Buff[Offset+8])+(256*256*Buff[Offset+8+1])+(256*Buff[Offset+8+2])+Buff[Offset+8+3];//cvtDWord(Buff, Offset+8);
          tlen:=(256*256*256*Buff[Offset+12])+(256*256*Buff[Offset+12+1])+(256*Buff[Offset+12+2])+Buff[Offset+12+3];//cvtDWord(Buff, Offset+12);
          {$ifdef OTM}
        ...
        {$endif}
          ...
          if tabTag='hhea' then begin
            ...
            PDFFont.NumMetrics:=(256*Buff[toff+tlen-2])+(Buff[toff+tlen-2+1]);//cvtInt(Buff, toff+tlen-2);
          end;
  ...
  begin
    ...
    if FFontHandling<>fhNoEmbedding then begin
  ...
  begin
    ...
    for K := 0 to FReportFonts.Count-1 do begin
      ...
      Inc(numfontobjs);
      if FEmbedFonts.IndexOf(FontName)>=0 then begin
        if FFontHandling=fhNoEmbedding then begin
...
{$endif}
    try
      ...
      kidstring:='/Kids [';
      for K := 0 to FPDF.pagetextfiles.Count-1 do kidstring:=kidstring+Format('%d 0 R ', [4+(
                              2*K)]);
...
begin
  ...
  try
    ...
    if (GControl is TQRGrImage) then begin
      if not TQRGrImage(GControl).ClipImage then tempmap.Canvas.StretchDraw(Rect(0, 0, tempmap.Width, tempmap.
                               Height), TQRGrImage(GControl).Picture.Graphic) else begin
        tempmap.Canvas.Draw(0, 0, TQRGrImage(GControl).Picture.Graphic)
      end;
      CaptureImageBytes(tempmap);
    end else if (GControl is TQRImage) then begin
      if TQRImage(GControl).Stretch then // QR4 remove Not
      ...
      CaptureImageBytes(tempmap);
    end else if (GControl is TQRDBImage) then begin
      QRDBCont:=GControl as TQRDBImage;
      if QRDBCont.Field=nil then Exit else begin
        DrawPict.Assign(QRDBCont.Field);
        //AspectRatio := 1.0 * drawpict.Width / drawpict.height;
        if not QRDBCont.Stretch then begin
    ...
    end else if (GControl is TQRGrDBImage) then begin
      dbcont:=GControl as TQRGrDBImage;
      if dbcont.Field=nil then Exit else begin
        ...
        if not dbcont.Stretch then begin
          ...
          end else if dbcont.FitMode=fmProportional then begin
            if (DrawPict.Width/conwidth)<(DrawPict.Height/conheight) then conheight:=trunc(conwidth/
                                      AspectRatio) else conwidth:=trunc(conheight*AspectRatio);
          end else if dbcont.FitMode=fmKeepWidth then conheight:=trunc(conwidth/AspectRatio) else if dbcont.FitMode
                            =fmKeepHeight then conwidth:=trunc(conheight*AspectRatio);
        end;
        //--------
        if not dbcont.ClipImage then tempmap.Assign(dbcont.Field) else begin
          tempmap.Canvas.Draw(0, 0, DrawPict.Graphic);
        end;
        CaptureImageBytes(tempmap);
      end;
