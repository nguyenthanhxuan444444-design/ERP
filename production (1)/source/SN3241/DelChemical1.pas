...
uses
  ...
  DBGridEh, Mask, Buttons, ExtCtrls ,IniFiles,comobj;
...
procedure TDelChemical.ReadIni();
var MyIni :Tinifile;
    AppDir:string;
begin
  //
...
begin
  ...
  //Synchronize LIY_DD.dbo.DDZL.YN from ERP
  with  Qtemp do
  begin
    Active:=false;
    SQL.Clear;
    ...
    SQL.Add('from (');
    SQL.Add('select DDZL.DDBH,DDZL.ZLBH1,ERP_DDZL.YN ');
    SQL.Add('from LIY_DD.dbo.DDZL DDZL');
    SQL.Add('inner join DDZL ERP_DDZL on ERP_DDZL.DDBH=DDZL.ZLBH1');
    SQL.Add('where  DDZL.YN is null ) ERP');
    SQL.Add('where ERP.DDBH=LIY_DD.dbo.DDZL.DDBH');
    SQL.Add('Update  LIY_DD.dbo.DDZL set YN=ERP.YN');
    SQL.Add('from (');
    SQL.Add('select DDZL.DDBH,DDZL.ZLBH1,ERP_YPZL.YN ');
    SQL.Add('from LIY_DD.dbo.DDZL DDZL');
    SQL.Add('inner join YPZL ERP_YPZL on ERP_YPZL.YPDH=DDZL.ZLBH1');
    SQL.Add('where  DDZL.YN is null  ) ERP');
    SQL.Add('where ERP.DDBH=LIY_DD.dbo.DDZL.DDBH');
    SQL.Add('update LIY_DD.dbo.DDZL');
    SQL.Add('set LIY_DD.dbo.DDZL.YN=1');
    SQL.Add('where  (LIY_DD.dbo.DDZL.ZLBH1 like ''AURR%'' or  LIY_DD.dbo.DDZL.ZLBH1 like ''GS%'' or LIY_DD.dbo.DDZL.ZLBH1 like ''SCI%'' or LIY_DD.dbo.DDZL.ZLBH1 like ''IMG%'')  and YN is null');
    // 20240226
    SQL.Add('Update  LIY_DD.dbo.DDZL set YN=ERP.YN ');
...
begin
  ...
  begin
    active:=false;
    sql.Clear;
    sql.Add('select KCLLS.*,CLZL.YWPM,CLZL.DWBH,CLZL.ZWPM ');
    if chk_old.Checked then
...
begin
  ...
  DelChemical_Dep.ParentQry:=DelChemical.DelMas;
...
begin
  ...
  begin
    ...
    DelChemical_CL.ParentQry:=DelChemical.DelDet;
    DelChemical_CL.show;
  end
  else
    begin
      DelChemical_Mat:=TDelChemical_Mat.create(self);
      DelChemical_Mat.ParentQry:=DelChemical.DelDet;
      DelChemical_Mat.CKBH:=DelChemical.DelMas.fieldbyname('CKBH').AsString;
...
begin
  ...
  begin
    ...
    DelChemical_SCBH.ParentQry:=DelChemical.DelDet;
...
procedure TDelChemical.BDT6Click(Sender: TObject);
  ...
  begin
    ...
    try
      ...
      for   i:=0   to   DBGrid4.fieldcount-1   do
        begin
            eclApp.Cells(1,i+1):=DBGrid4.fields[i].FieldName;
        end;
      DelDet.First;
      j:=2;
      while   not   DelDet.Eof   do
...
procedure TDelChemical.bbt6Click(Sender: TObject);
var eclApp,WorkBook:olevariant;
    i,j:integer;
begin
  if  DelMas.active  then
