...
procedure TDelChemical_Print.UpLoadQRImg(QRdata:string);
var
...
procedure TDelChemical_Print.ReadIni();
var MyIni :Tinifile;
    AppDir:string;
begin
  check:='';
...
begin
  ReadIni();
  ADate := DelChemical.DelMas.fieldbyname('USERDATE').Value;
  BDate := EncodeDate(2023, 6, 30);
  if (CompareDate(ADate,BDate)>0) and (check='TYXUAN') then
  begin
    QRCodeBitmap:=TBitmap.Create;
    UpLoadQRImg(DelChemical.DelMas.fieldbyname('LLNO').AsString);
