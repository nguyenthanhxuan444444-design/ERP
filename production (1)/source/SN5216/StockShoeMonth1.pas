...
begin
  ...
  begin
    Active:=false;
    SQL.Clear;
    SQL.Add(' if object_id(''tempdb..#YWCP'') is not null');
    SQL.Add(' begin drop table #YWCP end');
    ...
    SQL.Add('       DDZL.ShipDate,DDZL.ARTICLE,DDZL.DDZT,YWCPMemo.Memo,isnull(RK_CTS.CTS,0) - isnull(LL_CTS.CTS,0) as CTS,kfzl.kfjc as Customer,''Giay thanh pham'' as ModelName ');
    SQL.Add('       ,KCCLMONTH_Order.KVBH,KCCLMONTH_Order.STT, KHPO into #YWCP  ');
    SQL.Add('from (');
    ...
    SQL.Add('	Select Max(YWCP.KCBH) as CKBH,case when ddzltr.DDBH1 is not null then ddzltr.DDBH else YWCP.DDBH end DDBH,Sum(YWCP.Qty) as Qty,''2'' as SB');
    SQL.Add('	from (Select * from YWCP union all Select * from YWCPOld) YWCP');
    ...
    SQL.Add('	Group by YWCP.DDBH,ddzltr.DDBH1,ddzltr.DDBH ) YWCPIn Group by CKBH,DDBH,SB');
    SQL.Add('	union all');
    SQL.Add(' Select CKBH,DDBH,Sum(Qty) as Qty, SB From ( ');
    SQL.Add('	Select Max(YWCP.KCBH) as CKBH,case when ddzltr.DDBH1 is not null then ddzltr.DDBH else YWCP.DDBH end DDBH,-1*Sum( Case when SB=''3'' then YWCP.Qty else 0 end) as Qty,''3'' as SB');
    SQL.Add('	from (Select * from YWCP union all Select * from YWCPOld) YWCP');
    SQL.Add(' left join DDZL on YWCP.DDBH=DDZL.DDBH ');
    SQL.Add('	left join DDZLTR on ywcp.DDBH=ddzltr.DDBH1 ');
    ...
    SQL.Add(' and ywcp.DDBH not in (select DDBH from DDZLTR) ');
    SQL.Add('	Group by YWCP.DDBH,ddzltr.DDBH1,ddzltr.DDBH ) YWCPOut Group by CKBH,DDBH,SB ');
    SQL.Add('	) YWCPMONTH');
    ...
    SQL.Add('	where convert(smalldatetime,convert(varchar,YWCP.INDATE,111)) between '''+Formatdatetime('yyyy/MM/dd',startofthemonth(DTP.date))+''' and '''+Formatdatetime('yyyy/MM/dd',DTP.date)+''' and IsNull(SB,'''')<>'''' ');
    SQL.Add(' and ywcp.DDBH not in (select DDBH from DDZLTR) ');
    SQL.Add('	Group by YWCP.DDBH,ddzltr.DDBH1,ddzltr.DDBH ) YWCPIn Group by CKBH,DDBH,SB  ) YWCPRK on YWCP.DDBH=YWCPRK.DDBH');
    SQL.Add('  Left join  (');
    SQL.Add(' Select CKBH,DDBH,Sum(Qty) as Qty, SB From ( ');
    SQL.Add('	Select Max(YWCP.KCBH) as CKBH,case when ddzltr.DDBH1 is not null then ddzltr.DDBH else YWCP.DDBH end DDBH,Sum( Case when SB=''3'' then YWCP.Qty else 0 end) as Qty,''5'' as SB');
    SQL.Add('	from (Select * from YWCP union all Select * from YWCPOld) YWCP');
    SQL.Add(' left join DDZL on YWCP.DDBH=DDZL.DDBH ');
    SQL.Add('	left join DDZLTR on ywcp.DDBH=ddzltr.DDBH1 ');
    SQL.Add('	where convert(smalldatetime,convert(varchar,YWCP.EXEDATE,111)) between '''+Formatdatetime('yyyy/MM/dd',startofthemonth(DTP.date))+''' and '''+Formatdatetime('yyyy/MM/dd',DTP.date)+''' and IsNull(SB,'''')<>''''');
    SQL.Add(' and ywcp.DDBH not in (select DDBH from DDZLTR) ');
    SQL.Add('	Group by YWCP.DDBH,ddzltr.DDBH1,ddzltr.DDBH ) YWCPOut Group by CKBH,DDBH,SB ) YWCPXH on YWCP.DDBH=YWCPXH.DDBH');
    SQL.Add('   Left join (');
    //20221126 修改
    ...
    {if CKBHEdit.Text<>'' then
      SQL.Add(' and YWDDM.CKBH like '''+CKBHEdit.Text+'%'' ');
    if KVBHEdit.Text<>'' then
      SQL.Add(' and KCCLMONTH_Order.KVBH like '''+KVBHEdit.Text+'%'' '); }
    SQL.Add('Order by KCCLMONTH_Order.STT,YWDDM.YSBH ');
    ExecSQL();
    if checkbox4.Checked then
    begin
      SQL.Add('Select YWCP.DDBH,YWCP.YSBH,YWCP.CKBH,YWCP.Pairs,YWCP.LastRem,YWCP.RKQty,YWCP.XHQty,YWCP.TRQty,YWCP.Qty- Isnull(YWCP_Out.Qty,0) as Qty,YWCP.shipDate,YWCP.Article,YWCP.DDZT,YWCP.Memo ');
      SQL.Add('       ,YWCP.CTS-isnull(YWCP_Out.CTS,0) as CTS,YWCP.Customer, YWCP.ModelName,KVBH,STT');
    end else
    begin
      SQL.Add('Select DDBH,YSBH,CKBH, Pairs,LastRem,RKQty,XHQty,TRQty,Qty,ShipDate,ARTICLE,DDZT,Memo,CTS, Customer, ModelName,KVBH,STT, KHPO ');
    end;
    SQL.Add('From #YWCP YWCP');
    if checkbox4.Checked then
    begin
      SQL.Add('left join ( Select Max(YWCP.KCBH) as CKBH,case when ddzltr.DDBH1 is not null then ddzltr.DDBH else YWCP.DDBH end DDBH,Sum( Case when SB=''2'' then YWCP.Qty else 0 end) as Qty,''4'' as SB,count(*) as CTS ');
      SQL.Add('	           From ( Select * from YWCP union all Select * from YWCPOld) YWCP ');
      SQL.Add('                   Left join DDZL on YWCP.DDBH=DDZL.DDBH ');
      SQL.Add('                   Left join DDZLTR on ywcp.DDBH=ddzltr.DDBH1 ');
      SQL.Add('                 	Where DDZL.GSBH='''+main.Edit2.Text+''' and convert(smalldatetime,convert(varchar,YWCP.OUTDATE,111)) between '''+Formatdatetime('yyyy/MM/dd',startofthemonth(DTP.date))+''' and '''+Formatdatetime('yyyy/MM/dd',DTP.date)+''' and IsNull(SB,'''')=''2'' ');
      SQL.Add('                   and YWCP.DDBH not in (select DDBH from DDZLTR)');
      if DDBHEdit.Text<>'' then
      SQL.Add('                   and YWCP.DDBH like '''+DDBHEdit.Text+'%'' ');
      SQL.Add('                 	Group by YWCP.DDBH,ddzltr.DDBH1,ddzltr.DDBH ) YWCP_Out on YWCP.DDBH=YWCP_Out.DDBH ');
    end;
    SQL.Add('Where 1=1 ');
    if ((CheckBox1.Checked=true) and (checkbox4.Checked=False)) then
    begin
      SQL.Add('   and YWCP.Qty>0 ');
    end;
    if ((CheckBox1.Checked=true) and (checkbox4.Checked=True)) then
    begin
      SQL.Add('   and (YWCP.Qty- Isnull(YWCP_Out.Qty,0)) >0 ');
    end;  
    if CheckBox3.Checked=true then
    SQL.Add('   and YWCP.Qty<0 ');
    if CheckBox2.Checked=true then
    SQL.Add('   and YWCP.Qty is null ');
    if CKBHEdit.Text<>'' then
      SQL.Add(' and YWCP.CKBH like '''+CKBHEdit.Text+'%'' ');
    if KVBHEdit.Text<>'' then
      SQL.Add(' and YWCP.KVBH like '''+KVBHEdit.Text+'%'' ');
    SQL.Add('Order by YWCP.STT,YWCP.DDBH');
    FuncObj.WriteErrorLog(sql.Text);
    Active:=true;
  end;
end;
...
begin
    ...
    begin
      Active:=false;
      SQL.Clear;
      SQL.Add('Select STT, distinct YWDDM.DDBH,YWDDM.YSBH,MAX(KCLLS.SCBH )as KCLLS_SCBH,YWDDM.CKBH,IsNull(YWDD.Qty,DDZL.Pairs) as Pairs,YWDDM.LastRem,YWDDM.RKQty,YWDDM.XHQty,YWDDM.TRQty,YWDDM.Qty,');
      ...
      begin
      SQL.Add(' Select ROW_NUMBER() OVER (ORDER BY YWDDM.DDBH) AS STT,YSBH as DDBH,YSBH,CKBH,Sum(LastRem) as LastRem,Sum(RKQty) as RKQty,Sum(XHQty) as XHQty,Sum(TRQty) as TRQty,Sum(Qty) as Qty from ( ');
      end;
      SQL.Add('  Select ROW_NUMBER() OVER (ORDER BY YWDDM.DDBH) AS STT,YWCP.DDBH,IsNull(YWDD.YSBH,YWCP.DDBH) as YSBH,Max(YWCP.CKBH) as CKBH,Sum(YWCPMonth.Qty) as LastRem,Sum(YWCPRK.Qty) as RKQty,');
      SQL.Add('         Sum(YWCPXH.Qty) as XHQty,SUM(YWCPMONTH_DDZLTR.Qty) as TRQty,Case when SUM(YWCP.Qty) is not null and SUM(YWCPMONTH_DDZLTR.Qty) is not null then SUM(YWCP.Qty)-SUM(YWCPMONTH_DDZLTR.Qty) else SUM(YWCP.Qty) end as Qty');
      SQL.Add('  from (');
      SQL.Add('	Select DDBH,Max(CKBH) as CKBH, Sum(Qty) as Qty from (');
      SQL.Add('    Select YWCPMONTH.CKBH,YWCPMONTH.DDBH,YWCPMONTH.Qty,''1'' as SB ');
