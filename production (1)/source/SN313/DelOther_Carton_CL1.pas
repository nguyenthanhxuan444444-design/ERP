...
uses
  ...
  Dialogs, DB, DBTables, GridsEh, DBGridEh, StdCtrls, ExtCtrls,IniFiles;
...
uses DelOther_Conton1, main1, FunUnit;

{$R *.dfm}
procedure TDelOther_Carton_CL.ReadIni();
var MyIni :Tinifile;
    AppDir:string;
begin
  Pro_CostID:='6211';
...
begin
  ...
  begin
    ...
    sql.add('from ');
    sql.add('(select DDBH,CLBH,Sum(CTS) as CTS  from YWBZPO where DDBH in (select DDBH from YWDD where YSBH like '''+edit1.Text+'%'' Group by DDBH)  Group by DDBH,CLBH ');
    sql.add(' Union all ');
    sql.add(' select DDBH,CLBH,Sum(CTS) as CTS  from YWBZPOE where DDBH in (select DDBH from YWDD where YSBH like '''+edit1.Text+'%'' Group by DDBH)  Group by DDBH,CLBH ) YWBZPO ');
    ...
    begin
        sql.add('            and KCLL.USERDate>='+''''+formatdatetime('yyyy/mm/dd',IncMonth(NDate,-6))+'''');
    end;
...
procedure TDelOther_Carton_CL.FormCreate(Sender: TObject);
begin
  ReadIni();
end;
