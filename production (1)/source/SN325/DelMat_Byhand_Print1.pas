...
type
    TDelMat_Byhand_Print = class(TForm)
    QuickRep1: TQuickRep;
...
var
  DelMat_Byhand_Print: TDelMat_Byhand_Print;
...
procedure TDelMat_Byhand_Print.ReadIni();
var MyIni :Tinifile;
    AppDir:string;
begin
  //
...
procedure TDelMat_Byhand_Print.UpLoadQRImg(QRdata:string);
var
...
procedure TDelMat_Byhand_Print.FormCreate(Sender: TObject);
var  ADate, BDate : TDateTime;
begin
  ReadIni();
  ADate := DelMat_Byhand.DelMas.fieldbyname('USERDATE').Value;
  BDate := EncodeDate(2023, 6, 30);
  if (CompareDate(ADate,BDate)>0) and (check='TYXUAN') then
  begin
    QRCodeBitmap:=TBitmap.Create;
    UpLoadQRImg(DelMat_Byhand.DelMas.fieldbyname('LLNO').AsString);
...
procedure TDelMat_Byhand_Print.DelDetCalcFields(DataSet: TDataSet);
begin
...
procedure TDelMat_Byhand_Print.FormDestroy(Sender: TObject);
begin
  DelMat_Byhand_Print:=nil;
  QRCodeBitmap.Free;
end;

procedure TDelMat_Byhand_Print.FormClose(Sender: TObject;
