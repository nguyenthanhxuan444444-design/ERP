...
var
  DelOther_O_Print: TDelOther_O_Print;
...
procedure TDelOther_O_Print.ReadIni();
var MyIni :Tinifile;
    AppDir:string;
...
procedure TDelOther_O_Print.UpLoadQRImg(QRdata:string);
var
...
procedure TDelOther_O_Print.FormCreate(Sender: TObject);
...
begin
  ...
  begin
    ...
    begin
      ...
      SQL.Add('  MAX(CASE WHEN ((S_STEP_DESC = ''部門主管'') or (S_STEP_DESC = ''單位主管'' and LEVEL_NO>10) or (S_STEP_DESC = ''申請人'' and LEVEL_NO>10) ) THEN S_USER_ID END) AS ManagerID,    ');
      SQL.Add('  MAX(CASE WHEN ((S_STEP_DESC = ''部門主管'') or (S_STEP_DESC = ''單位主管'' and LEVEL_NO>10) or (S_STEP_DESC = ''申請人'' and LEVEL_NO>10) ) THEN USERNAME END) AS Manager,    ');
      SQL.Add('  MAX(CASE WHEN ((S_STEP_DESC = ''部門主管'') or (S_STEP_DESC = ''單位主管'' and LEVEL_NO>10) or (S_STEP_DESC = ''申請人'' and LEVEL_NO>10) ) THEN ApproveDate END) AS ManagerDate,    ');
      SQL.Add('  MAX(CASE WHEN ((S_STEP_DESC = ''廠主管'') or (S_STEP_DESC = ''部門主管'' and LEVEL_NO>20) or (S_STEP_DESC = ''單位主管'' and LEVEL_NO>20) or (S_STEP_DESC = ''申請人'' and LEVEL_NO>20) )  THEN S_USER_ID END) AS DirectorID,    ');
      SQL.Add('  MAX(CASE WHEN ((S_STEP_DESC = ''廠主管'') or (S_STEP_DESC = ''部門主管'' and LEVEL_NO>20) or (S_STEP_DESC = ''單位主管'' and LEVEL_NO>20) or (S_STEP_DESC = ''申請人'' and LEVEL_NO>20) ) THEN USERNAME END) AS Director,    ');
      SQL.Add('  MAX(CASE WHEN ((S_STEP_DESC = ''廠主管'') or (S_STEP_DESC = ''部門主管'' and LEVEL_NO>20) or (S_STEP_DESC = ''單位主管'' and LEVEL_NO>20) or (S_STEP_DESC = ''申請人'' and LEVEL_NO>20) ) THEN ApproveDate END) AS DirectorDate,    ');
      SQL.Add('  MAX(CASE WHEN S_STEP_DESC = ''企劃主管'' THEN S_USER_ID END) AS Approver1ID,  ');
      SQL.Add('  MAX(CASE WHEN S_STEP_DESC = ''企劃主管'' THEN USERNAME END) AS Approver1,  ');
      SQL.Add('  MAX(CASE WHEN S_STEP_DESC = ''企劃主管'' THEN ApproveDate END) AS Approver1Date,  ');
      SQL.Add('  MAX(CASE WHEN S_STEP_DESC = ''總務單位主管'' THEN S_USER_ID END) AS Supvervisor1ID,  ');
      SQL.Add('  MAX(CASE WHEN S_STEP_DESC = ''總務單位主管'' THEN USERNAME END) AS Supvervisor1,  ');
      SQL.Add('  MAX(CASE WHEN S_STEP_DESC = ''總務單位主管'' THEN ApproveDate END) AS Supvervisor1Date,  ');
      SQL.Add('  MAX(CASE WHEN S_STEP_DESC = ''總審核'' THEN S_USER_ID END) AS Approver2ID,  ');
      SQL.Add('  MAX(CASE WHEN S_STEP_DESC = ''總審核'' THEN USERNAME END) AS Approver2,  ');
      SQL.Add('  MAX(CASE WHEN S_STEP_DESC = ''總審核'' THEN ApproveDate END) AS Approver2Date  ');
      SQL.Add('       FROM (  ');
      SQL.Add('      ');
      ...
      SQL.Add('      WHERE FORM_TABLE = ''KCLL'' AND STATUS IN (''N'', ''Z'') AND FORM_PRESENTATION LIKE ''%' + DelOther_O.DelMas.FieldByName('LLNO').AsString + '%''  ');
      ...
      SQL.Add('  MAX(CASE WHEN GROUP_NAME=APPGROUP AND ((LEV=4 AND APPGROUP_LEV=4 AND TITLE_NAME<>''越籍人員'') OR (LEV=5 AND APPGROUP_LEV=5 AND TITLE_NAME<>''越籍人員'')) THEN ACCOUNT END) AS SupvervisorID, ');
      SQL.Add('  MAX(CASE WHEN GROUP_NAME=APPGROUP AND ((LEV=4 AND APPGROUP_LEV=4 AND TITLE_NAME<>''越籍人員'') OR (LEV=5 AND APPGROUP_LEV=5 AND TITLE_NAME<>''越籍人員'')) THEN NAME END) AS Supvervisor,   ');
      SQL.Add('  MAX(CASE WHEN GROUP_NAME=APPGROUP AND ((LEV=4 AND APPGROUP_LEV=4 AND TITLE_NAME<>''越籍人員'') OR (LEV=5 AND APPGROUP_LEV=5 AND TITLE_NAME<>''越籍人員'')) THEN FINISH_TIME END) AS SupvervisorDate,   ');
      SQL.Add('  MAX(CASE WHEN LEV=3 THEN ACCOUNT END) AS ManagerID,     ');
      SQL.Add('  MAX(CASE WHEN LEV=3 THEN NAME END) AS Manager,     ');
      SQL.Add('  MAX(CASE WHEN LEV=3 THEN FINISH_TIME END) AS ManagerDate,     ');
      SQL.Add('  MAX(CASE WHEN LEV=3 THEN ACCOUNT END) AS DirectorID,     ');
      SQL.Add('  MAX(CASE WHEN LEV=3 THEN NAME END) AS Director,     ');
      SQL.Add('  MAX(CASE WHEN LEV=3 THEN FINISH_TIME END) AS DirectorDate,     ');
      SQL.Add('  MAX(CASE WHEN GROUP_NAME = ''企劃 KH'' THEN ACCOUNT END) AS Approver1ID,  ');
      SQL.Add('  MAX(CASE WHEN GROUP_NAME = ''企劃 KH'' THEN NAME END) AS Approver1,  ');
      SQL.Add('  MAX(CASE WHEN GROUP_NAME = ''企劃 KH'' THEN FINISH_TIME END) AS Approver1Date,  ');
      SQL.Add('  MAX(CASE WHEN GROUP_NAME = ''總務 A.VPTV'' AND TITLE_NAME<>''越籍人員'' THEN ACCOUNT END) AS Supvervisor1ID,  ');
      SQL.Add('  MAX(CASE WHEN GROUP_NAME = ''總務 A.VPTV'' AND TITLE_NAME<>''越籍人員'' THEN NAME END) AS Supvervisor1,  ');
      SQL.Add('  MAX(CASE WHEN GROUP_NAME = ''總務 A.VPTV'' AND TITLE_NAME<>''越籍人員'' THEN FINISH_TIME END) AS Supvervisor1Date,  ');
      ...
      SQL.Add('             ROW_NUMBER() OVER (PARTITION BY FINISH_TIME ORDER BY FINISH_TIME DESC) AS RowID, TB_EB_GROUP.LEV, APPGROUP.LEV APPGROUP_LEV, APPGROUP.GROUP_NAME APPGROUP ');
      SQL.Add('      FROM [UOF].[dbo].[TB_WKF_TASK] TB_WKF_TASK  ');
      SQL.Add('      LEFT JOIN [UOF].[dbo].[LYN_FormSigner] LYN_FormSigner on LYN_FormSigner.TableName=''''KCLL'''' and LYN_FormSigner.Type=''''XXXXXXXXXX'''' and LYN_FormSigner.FNO=TB_WKF_TASK.DOC_NBR ');
      ...
      if (FileExists(SignImgPath + FieldByName('DirectorID').AsString + '.bmp')) then
      begin
        DirectorSign.Picture.LoadFromFile(SignImgPath + FieldByName('DirectorID').AsString + '.bmp');
        QRDBText13.DataField := '';
      end;
    end;
  end
...
procedure TDelOther_O_Print.FormDestroy(Sender: TObject);
begin
  DelOther_O_Print:=nil;
  QRCodeBitmap.Free;
end;

procedure TDelOther_O_Print.FormClose(Sender: TObject;
