...
uses ScanReturn_hand1;

{$R *.dfm}

procedure TScanReturn.FormDestroy(Sender: TObject);
begin
  ScanReturn:=nil;
end;

procedure TScanReturn.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TScanReturn.BB3Click(Sender: TObject);
begin
  ScanData.delete;
end;

procedure TScanReturn.BB1Click(Sender: TObject);
var i:integer;
    s:string;
begin
  ...
  begin
      ScanReturn_hand:= TScanReturn_hand.create(self);
      ScanReturn_hand.show;
  end;

end;

procedure TScanReturn.BB4Click(Sender: TObject);
begin
  Close;
end;

procedure TScanReturn.BB2Click(Sender: TObject);
var i:integer;
    CARTONBAR:string;
    DDBH:string;
begin
          ...
          usinserted:
            begin
              ...
              begin
                  ...
                  sql.Clear;
                  sql.add('update YWCP');
                  sql.add('set RECS=RECS+1 ');
                  sql.add('    ,REDATE=getdate()');
                  sql.add('    ,SB=''1'' ');
                  sql.add('where CARTONBAR='+''''+CARTONBAR+'''');
                  sql.add('      and RECS is not null  ');
                  sql.add('update YWCP');
                  sql.add('set RECS=1 ');
                  sql.add('    ,REDATE=getdate()');
                  sql.add('    ,SB=''1'' ');
                  sql.add('where CARTONBAR='''+CARTONBAR+'''');
                  sql.add('      and RECS is  null  ');
...
procedure TScanReturn.BitBtn1Click(Sender: TObject);
begin
...
procedure TScanReturn.Button1Click(Sender: TObject);
begin
  Panel1.Visible:=false;
end;

procedure TScanReturn.Edit1KeyPress(Sender: TObject; var Key: Char);
var s:string;
begin
  if key=#13 then
  begin
     //20171110 UCC標 weston修改
    if YWCP.Locate('CARTONBAR',RightStr(edit1.Text,20),[]) then
    begin
...
procedure TScanReturn.CBX1Click(Sender: TObject);
begin
  if CBX1.checked then
  begin
    YWBZPOS.Active:=true;
...
procedure TScanReturn.ScanDataAfterDelete(DataSet: TDataSet);
begin
  edit2.text:=inttostr(Scandata.RecordCount);
end;

procedure TScanReturn.FormCreate(Sender: TObject);
begin
  YWCP.Active:=true;
  SCANDATA.active:=true;
end;
