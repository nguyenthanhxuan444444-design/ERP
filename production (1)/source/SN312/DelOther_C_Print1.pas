...
uses
  ...
  Dialogs, ExtCtrls, QuickRpt, QRCtrls, StdCtrls, QRPDFFilt, DBTables, DB,
...
procedure TDelOther_C_Print.UpLoadQRImg(QRdata:string);
var
...
procedure TDelOther_C_Print.ReadIni();
var MyIni :Tinifile;
    AppDir:string;
begin
  check:='';
...
procedure TDelOther_C_Print.FormCreate(Sender: TObject);
var  ADate, BDate : TDateTime;
begin
  ReadIni();
  ADate := DelOther_C.DelMas.fieldbyname('USERDATE').Value;
  BDate := EncodeDate(2023, 6, 30);
  if (CompareDate(ADate,BDate)>0) and (check='TYXUAN') then
  begin
    QRCodeBitmap:=TBitmap.Create;
    UpLoadQRImg(DelOther_C.DelMas.fieldbyname('LLNO').AsString);
    QRImage1.Picture.Assign(QRCodeBitmap);
  end;
    with  DelTot do
    begin
      active:=true;
      DelOther_C.DelDet.first;
      while not DelOther_C.DelDet.eof do
      begin
          if locate('CLBH',DelOther_C.DelDet.FieldByName('CLBH').value,[]) then
          begin
              edit;
              fieldbyname('Memo').Value:=fieldbyname('Memo').asstring+'// '
                              +DelOther_C.DelDet.fieldbyname('SCBH').AsString+'('+DelOther_C.DelDet.fieldbyname('Qty').AsString+')';
              post;
          end;
          DelOther_C.DelDet.next;
      end;
      DelOther_C.DelDet.first;
    end;
